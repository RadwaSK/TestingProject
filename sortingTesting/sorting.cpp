...
void sorting::swapItems(int& a, int& b) {
	int temp = b;
	b = a;
	a = temp;
}
...
int sorting::findLargestIndex(const int* arr, int size) {
	if (size == 0) return -1;
...
int sorting::MakePartation(int* arr, int size) {
	if (size < 2) return 0;
...
//------------------------------------------------------------------------------------------------------------->
void sorting::QuickSort3(int arr[], int l, int r)
{
